...
1. **Variant.** Choose between `cachy-server`, `cachy-kde`, `cachy-gnome`, `cachy-hyprland`, `arch-server`, `arch-kde`, `arch-gnome`, or `arch-hyprland`. The picker fetches the live variant catalog from this repository so the choices stay current. See [Variants](../variants/) for details on each.
