...
# DKMS module signing is disabled (see /etc/dkms/framework.conf:
# try_sign_modules='false') so modules ride the UKI Authenticode
# signature instead. The mok.key/pub files shouldn't be created at
# all under that policy — but rm -f as defense in depth in case some
# upstream code path generates them anyway.
echo "==> Removing any DKMS mok keypair (should be no-op under try_sign_modules=false)"
